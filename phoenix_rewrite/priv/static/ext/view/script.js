...
function ensureTagEditorTagit() {
    if (!window.jQuery || !window.jQuery.fn || !window.jQuery.fn.tagit) {
        return;
    }

    const $ = window.jQuery;
    const $editor = $("#tag_editor");
    if ($editor.length === 0) return;

    const alreadyTagit =
        $editor.hasClass("tagit-hidden-field") ||
        $editor.prevAll("ul.tagit").length > 0 ||
        $editor.nextAll("ul.tagit").length > 0;
    if (alreadyTagit) return;
    ...
    $editor.tagit({
        ...
        autocomplete: {
            source: function (request, response) {
                $.ajax({
                    url: (window.base_href || "") + "/api/internal/autocomplete",
                    data: { s: request.term },
                    dataType: "json",
                    type: "GET",
...
function clearViewMode() {
    document.querySelectorAll(".image_info").forEach((element) => {
        ...
        element.classList.add("shm-edit-mode");
    });
    document.querySelectorAll(".image_info textarea").forEach((el) => {
        autosize(el);
    });
    if (typeof window.shmInitTagitAutocomplete === "function") {
        setTimeout(() => window.shmInitTagitAutocomplete(), 0);
    }
    setTimeout(() => ensureTagEditorTagit(), 0);
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".image_info").forEach((element) => {
        ...
        element.classList.remove("shm-edit-mode");
    });

    document.querySelectorAll(".shm-post-edit-toggle").forEach((el) => {
    ...
    window.clearViewMode = clearViewMode;
});
