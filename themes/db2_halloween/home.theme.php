...
EOD
    ...
    public function build_body(string $sitename, string $main_links, string $main_text, string $contact_link, $num_comma, string $counter_text): string
    {
      ...
      $data_href = get_base_href();
        ...
        return "
		<div id='front-page'>
			<h1><a style='text-decoration: none; color: #e59649;'href='".make_link()."'><span>{$sitename}</span></a></h1>
			<img class='litecustom-logo' src='{$data_href}/themes/{$theme_name}/logo_halloween.png'/>
			$main_links_html
			$search_html
			$message_html
			$counter_html
